...
const formSchema = z.object({
  ...
  colorId: z.string().min(1, { message: 'این قسمت نمی‌تواند خالی باشد' }),
  sizeId: z.string().min(1, { message: 'این قسمت نمی‌تواند خالی باشد' }),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onDelete = async () => {
    try {
      ...
      router.refresh()
      router.push(`/${params.storeId}/products`)
      toast({ title: 'محصول حذف شد.', variant: 'default' })
    } catch (error: any) {
      toast({
